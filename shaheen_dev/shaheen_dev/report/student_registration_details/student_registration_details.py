...
@frappe.whitelist()
def get_masjids_with_status(doctype=None, txt=None, searchfield=None, start=None, page_length=None, filters=None, **kwargs):
    """
    Returns a list of Masjids that have data matching the selected status.
    """
    ...
    if not status or status == "All":
        # If no specific status is selected, fetch all Masjids with data
        query = """
            SELECT DISTINCT
                masjid_name AS value, masjid_name AS label
            FROM
                `tabStudent Registration`
            WHERE
                masjid_name IS NOT NULL
        """
    else:
        # Fetch Masjids that have data for the selected status
        query = f"""
            SELECT DISTINCT
                masjid_name AS value, masjid_name AS label
            FROM
            ...
            WHERE
                status = %s
                AND masjid_name IS NOT NULL
        """
        # Execute the query with the selected status as a parameter
        return frappe.db.sql(query, (status,), as_dict=True)
